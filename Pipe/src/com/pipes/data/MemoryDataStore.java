...
public class MemoryDataStore implements IDataStore {
	...
	public Handle createHandle(long userId,  String contactMetaData, String properties) {
		...
		Handle handle = new Handle(handleIdGen.getAndIncrement(), userId,
								token, properties, contactMetaData);
		handles.put(handle.getHandleId(), handle);
		return handle;
	}

	@Override
	public Pipe createPipe(long userId,  String contactMetaData, 
		...
		// create pipe
		Pipe pipe = new Pipe(pipeIdGen.getAndIncrement(), userId,
				neededHandle.getUserId(),
				newProperties);
		...
		Contact c1 = new Contact(contactIdGen.getAndIncrement(),
							userId, pipe.getPipeId(),
							contactMetaData, ph1, ph2);
		Contact c2 = new Contact(contactIdGen.getAndIncrement(),
				neededHandle.getUserId(), pipe.getPipeId(),
